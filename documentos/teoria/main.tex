...
\noindent La proposición $p = n^2 \geq 1; \forall n \in \mathbb{N}$ es verdadera para todos los números naturales pero la proposición $q = n^2 \geq 4; \forall n \in \mathbb{N}$ es falsa pues $\exists n \in \mathbb{N}$ tal que $n < 4$, como es el caso de $n = 1$.
\bigskip
...
Consiste en demostrar que la implicación $P(k) \to P(k + 1)$ es verdadera.
\end{itemize}
\bigskip
...
\indent Se llama \textbf{Hipótesis inductiva} a \textbf{P(k)}, es decir la proposición o afirmación evaluada para \textbf{n=k}. Se asume que la hipótesis inductiva es Verdadera.
\bigskip

\indent Se llama \textbf{Tesis inductiva} a \textbf{P(k + 1)}, es decir la proposición o afirmación evaluada para \textbf{n=k+1}.

\subsubsection{Enunciado: Primer Principio de Inducción Matemática, Débil o Clásica}
\label{sec:enunciado-induccion-clasica}

Sea $P(n)$ una propiedad o proposición que se define para enteros $n$ y sea $a$ un entero fijo. Suponga que los siguientes dos enunciados son verdaderos:
...
\begin{itemize}
    \item \textbf{Establecimiento de las Premisas Axiomáticas:} Se define un conjunto $S$ que cumple estrictamente con las dos condiciones fundamentales de la inducción matemática: contiene al elemento $1$ (paso básico), y si contiene a un elemento $k$, obligatoriamente contiene a su sucesor $k+1$ (paso inductivo). El objetivo natural sería concluir que $S = \mathbb{N}$.
...
\noindent Sea $S \subseteq \mathbb{N}$ con $S \neq \emptyset$ \\
\indent $\cdot$ \textbf{Caso 1:} $S$ tiene elemento mínimo. \\
\indent $\cdot$ \textbf{Caso 2:} $S$ no tiene elemento mínimo. \\
\noindent Consideremos la propiedad $P(m) :=$ ``los naturales del $1$ al $m$ no están en $S$''. \\
\noindent Sea $B = \{m \in \mathbb{N} \mid P(m) \text{ se cumple}\}$. Entonces:

\begin{enumerate}
    \item \textbf{$1 \in B$} \\
    Notemos que si $1 \in S \Rightarrow 1$ es elemento mínimo \textit{(Contradicción)}. \\
    $\therefore 1 \notin S$ y entonces $P(1)$ se cumple y $1 \in B$.
    \item \textbf{$n \in B \Rightarrow n+1 \in B$} \\
    Supongamos que $n \in B \Rightarrow P(n)$ se cumple $\Rightarrow$ los naturales de $1$ a $n$ no están en $S$. \\
    $\Rightarrow n+1 \notin S$, pues si $n+1 \in S$ pero $1, 2, \dots, n \notin S$, tendríamos que $n+1$ es un mínimo de $S$. \\
    $\Rightarrow$ los naturales entre $1$ y $n+1$ no están en $S \Rightarrow P(n+1)$ se cumple $\Rightarrow n+1 \in B$.
\end{enumerate}

\noindent Por PIM tenemos que $B = \mathbb{N}$. Notemos que si $m \in B \Rightarrow m \notin S$. Pero como $B = \mathbb{N}$ y $S \subseteq \mathbb{N} \Rightarrow S = \emptyset$ \textit{(Contradicción)}. \\
\noindent $\therefore S$ sí tiene elemento mínimo. 

\subsubsection{Análisis de la Implicación (PIM Implica PBO)}
\label{sec:analisis-pim-pbo}
\begin{itemize}    

\item \textbf{Estrategia Metodológica:} La estructura argumentativa se basa en una dicotomía y una posterior prueba por contradicción (\textit{reductio ad absurdum}). Se postula un conjunto $S \subseteq \mathbb{N}$ no vacío. El análisis se bifurca: el conjunto posee un mínimo, o carece de él. Al asumir la ausencia de mínimo, se inicia un proceso lógico diseñado para generar una incompatibilidad fundamental.
\item \textbf{El Predicado de Exclusión Acumulativa:} El eje de la demostración es la formulación de la propiedad $P(m)$: ``los naturales del $1$ al $m$ no están en $S$'', y la creación del conjunto auxiliar $B$. Esta técnica transforma una evaluación individual de elementos en una exclusión acumulativa, permitiendo que la Inducción Simple actúe con la robustez de la Inducción Fuerte.
\item \textbf{Evaluación del Caso Base:} Si el número $1$ perteneciera a $S$, actuaría como el mínimo absoluto, violando la premisa. Se deduce inexorablemente que $1 \notin S$, validando la pertenencia de $1$ al conjunto $B$.
\item \textbf{Transición Inductiva:} Bajo la hipótesis de que ningún natural hasta $n$ pertenece a $S$, la inclusión de $n+1$ en $S$ lo convertiría instantáneamente en el elemento mínimo. Para evitar esta contradicción con la premisa, $n+1$ debe quedar excluido de $S$, garantizando su ingreso al conjunto $B$.
\item \textbf{Conclusión por Reducción al Absurdo:} La inducción concluye que el conjunto excluyente $B$ abarca la totalidad de los naturales ($B = \mathbb{N}$). En consecuencia, el conjunto original $S$ carece de elementos ($S = \emptyset$), lo cual infringe directamente la hipótesis inicial ($S \neq \emptyset$). El colapso del Caso 2 demuestra que todo subconjunto no vacío de $\mathbb{N}$ debe poseer un elemento mínimo.
\end{itemize}
